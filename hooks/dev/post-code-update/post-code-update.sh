...
if [ "$target_env" = 'dev' ]; then
    ...
    echo "Copying test (aka stage) environment database to $target_env."
    cd $target_docroot && drush9 @${site}.test ac-database-copy ${site} ${target_env}
    ...
    cd $target_docroot && drush9 $drush_alias cdel views.view.migrate_paragraphs
    cd $target_docroot && drush9 $drush_alias cdel views.view.migrate_taxonomy
    cd $target_docroot && drush9 $drush_alias en migrate devel hal migrate_drupal migrate_drupal_ui migrate_source_csv migrate_upgrade bos_migration stage_file_proxy -y
