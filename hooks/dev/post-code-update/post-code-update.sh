...
if [ "$target_env" = 'dev' ]; then
    ...
    cd /var/www/html/bostond8.dev/docroot && drush9 $drush_alias pmu config_split -y
    cd /var/www/html/bostond8.dev/docroot && drush9 $drush_alias cdel core.extension module.config_split -y

    echo "Import configurations from $source_branch."
    cd /var/www/html/bostond8.dev/docroot && drush9 $drush_alias cim -y

    #    DEV ONLY - enable migration modules.
    echo "DEVELOPMENT PRE-D8 LAUNCH ONLY: Add back in some dev modules."
    cd /var/www/html/bostond8.dev/docroot && drush9 $drush_alias en migrate -y

    echo "Update $site.$target_env DB with un-executed hook_updates on deploy target."
    cd /var/www/html/bostond8.dev/docroot && drush9 $drush_alias updb -y

    echo "Update existing entities on $site.$target_env (poss. redundant)."
    cd /var/www/html/bostond8.dev/docroot && drush9 $drush_alias entup -y

    echo "Rebuild user content access permissions on $site.$target_env."
    cd /var/www/html/bostond8.dev/docroot && drush9 $drush_alias php-eval 'node_access_rebuild()' -y

    echo "Run cron now on $site.$target_env (poss. redundant)."
    cd /var/www/html/bostond8.dev/docroot && drush9 $drush_alias cron
