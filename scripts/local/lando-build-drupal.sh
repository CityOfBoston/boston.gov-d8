...
if [[ "${build_local_database_source}" == "none" ]]; then
  ...
  if [[ "${build_local_database_source}" == "initialize" ]]; then
    printout "INFO" "Switching to Initialize Mode and will create a new, empty database."
  fi
fi
...
if [[ "${build_local_database_source}" == "initialize" ]]; then
  ...
  printout "INFO" " ... with ${lando_services_database_type=mysql} database '${lando_services_database_creds_database}' on '${lando_services_database_host}:${lando_services_database_port}' in container '${LANDO_APP_PROJECT}_database_1'"
