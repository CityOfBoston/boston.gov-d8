...
class ScriptHandler {
  ...
  /**
   * Creates required directories and files for a Drupal installation.
   *
   * @param Event $event
   *   The Composer event.
   */
  ...
  public static function checkComposerVersion(Event $event) {
    ...
    // Check if Composer version is a placeholder or a development version
    if (preg_match('/^[0-9a-f]{40}$/i', $version)) {
      $version = $composer::BRANCH_ALIAS_VERSION;
    }

    // If Composer version is a placeholder or a development version, output a warning
    if ($version === '@package_version@' || $version === '@package_branch_alias_version@') {
      $io->writeError('<warning>You are running a development version of Composer. If you experience problems, please update Composer to the latest stable version.</warning>');
    }
    // If Composer version is less than 1.0.0, output an error
